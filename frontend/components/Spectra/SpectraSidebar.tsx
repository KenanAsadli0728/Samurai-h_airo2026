...
import { Layers, Upload, FileText, ChevronDown, ChevronUp, Thermometer, Lightbulb, Zap, Activity, ShieldAlert, Building2, Gavel } from 'lucide-react'
import { SpectraData, StressType, ConstructionViolation } from '@/lib/types'
...
interface SpectraSidebarProps {
  ...
  onBlindSpotLayerToggle: () => void
  selectedId: string | null
  violations: ConstructionViolation[]
  violationLayer: boolean
  onViolationLayerToggle: () => void
  selectedViolationId: string | null
  onSelectViolation: (id: string | null) => void
  onGenerateNotice: (violation: ConstructionViolation) => void
  onGenerateReport: () => void
}

const VIOLATION_STYLES: Record<ConstructionViolation['violation_type'], { label: string; badge: string }> = {
  unpermitted: { label: 'Unpermitted', badge: 'bg-red-500/15 border-red-500/30 text-red-400' },
  oversized:   { label: 'Oversized',   badge: 'bg-orange-500/15 border-orange-500/30 text-orange-400' },
  compliant:   { label: 'Compliant',   badge: 'bg-emerald-500/15 border-emerald-500/30 text-emerald-400' },
}
...
export default function SpectraSidebar({
  spectraData, esiHeatmap, onEsiHeatmapToggle, blindSpotLayer, onBlindSpotLayerToggle, selectedId,
  violations, violationLayer, onViolationLayerToggle, selectedViolationId, onSelectViolation, onGenerateNotice,
  onGenerateReport,
}: SpectraSidebarProps) {
  ...
  const blindSpots = hoods.filter(n => n.grid_blind_spot_score >= 40).length

  const avgUrbanRisk = Math.round(hoods.reduce((s, n) => s + n.urban_risk_score, 0) / total)
  const topRiskZone = [...hoods].sort((a, b) => b.urban_risk_score - a.urban_risk_score)[0]
  const violationCounts = {
    unpermitted: violations.filter(v => v.violation_type === 'unpermitted').length,
    oversized: violations.filter(v => v.violation_type === 'oversized').length,
    compliant: violations.filter(v => v.violation_type === 'compliant').length,
  }
  ...
  return (
    <aside data-testid="spectra-sidebar" className="flex-none w-80 bg-navy-900 border-r border-navy-600 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {/* Urban Risk Surface — fusion of energy stress + grid blind spots + predicted construction risk */}
        <div data-testid="urban-risk-surface" className="bg-gradient-to-br from-indigo-500/15 to-red-500/10 border border-indigo-500/30 rounded-xl p-3.5">
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs font-bold uppercase tracking-widest text-indigo-300 font-plex">Urban Risk Surface</p>
            <span className="text-2xl font-outfit font-bold text-white leading-none">{avgUrbanRisk}</span>
          </div>
          <p className="text-xs text-slate-400 font-plex leading-relaxed">
            Composite of energy stress, grid monitoring blind spots, and predicted construction-violation risk.
          </p>
          {topRiskZone && (
            <div className="flex items-center justify-between mt-2.5 pt-2 border-t border-indigo-500/20">
              <span className="text-xs text-slate-400 font-plex">Highest risk zone</span>
              <span className="text-xs font-semibold text-white font-plex">{topRiskZone.name} · {topRiskZone.urban_risk_score}</span>
            </div>
          )}
        </div>
        ...
        {selected && (
          <Section title="Selected Zone">
            <div className="bg-navy-800 border border-indigo-500/20 rounded-xl p-3 space-y-2">
              ...
              <div className="border-t border-navy-700 pt-2 mt-1 space-y-1.5">
                {[
                  { l: 'Metering Coverage', v: `${selected.metering_coverage_pct}%` },
                  { l: 'Grid Blind-Spot Risk', v: `${selected.grid_blind_spot_score}/100` },
                ].map(({ l, v }) => (
                  <div key={l} className="flex justify-between text-xs">
                    <span className="text-slate-400 font-plex">{l}</span>
                    <span className={`font-mono ${selected.grid_blind_spot_score >= 40 && l.includes('Blind') ? 'text-red-400' : 'text-white'}`}>{v}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {/* Grid Security Blind Spots (cybersecurity layer) */}
        <Section title="Grid Security">
          <div className="bg-navy-800 border border-red-500/20 rounded-xl p-3 mb-2">
            <p className="text-slate-400 text-xs font-plex leading-relaxed">
              Zones with <span className="text-white font-semibold">high energy stress</span> and{' '}
              <span className="text-white font-semibold">low metering coverage</span> are blind spots:
              anomalous load, grid tampering, or energy theft here goes undetected the longest.
            </p>
            <div className="flex items-center justify-between mt-2.5">
              <span className="text-xs text-slate-500 font-plex">Flagged zones (risk ≥ 40)</span>
              <span className={`px-1.5 py-0.5 rounded text-xs font-bold border font-plex ${
                blindSpots > 0 ? 'bg-red-500/15 border-red-500/30 text-red-400' : 'bg-navy-700 border-navy-600 text-slate-400'
              }`}>{blindSpots}</span>
            </div>
          </div>
          <button
            data-testid="layer-blind-spot"
            onClick={onBlindSpotLayerToggle}
            className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg border text-left transition-all duration-200 ${
              blindSpotLayer
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-xs font-semibold font-plex">Blind Spot Overlay</div>
                <div className="text-xs text-slate-500 mt-0.5 font-plex">Outlines unmonitored high-risk zones</div>
              </div>
            </div>
            <div className={`w-9 h-5 rounded-full transition-all duration-200 relative flex-shrink-0 ${blindSpotLayer ? 'bg-red-500' : 'bg-navy-600'}`}>
              <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-all duration-200 ${blindSpotLayer ? 'left-4' : 'left-0.5'}`} />
            </div>
          </button>
        </Section>

        {/* Construction Compliance (illegal construction / change detection) */}
        <Section title="Construction Compliance">
          <div className="bg-navy-800 border border-navy-600 rounded-xl p-3 mb-2">
            <p className="text-slate-400 text-xs font-plex leading-relaxed mb-2.5">
              Bi-temporal satellite change detection cross-referenced against the municipal permit registry.
            </p>
            <div className="grid grid-cols-3 gap-1.5">
              {([
                { key: 'unpermitted', label: 'Unpermitted' },
                { key: 'oversized', label: 'Oversized' },
                { key: 'compliant', label: 'Compliant' },
              ] as const).map(({ key, label }) => (
                <div key={key} className={`rounded-lg border px-2 py-1.5 text-center ${VIOLATION_STYLES[key].badge}`}>
                  <div className="font-outfit font-bold text-base leading-none">{violationCounts[key]}</div>
                  <div className="text-xs font-plex mt-0.5">{label}</div>
                </div>
              ))}
            </div>
          </div>
          <button
            data-testid="layer-violations"
            onClick={onViolationLayerToggle}
            className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg border text-left transition-all duration-200 ${
              violationLayer
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-xs font-semibold font-plex">Violation Overlay</div>
                <div className="text-xs text-slate-500 mt-0.5 font-plex">Flagged footprints vs. permit registry</div>
              </div>
            </div>
            <div className={`w-9 h-5 rounded-full transition-all duration-200 relative flex-shrink-0 ${violationLayer ? 'bg-orange-500' : 'bg-navy-600'}`}>
              <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-all duration-200 ${violationLayer ? 'left-4' : 'left-0.5'}`} />
            </div>
          </button>

          {/* Violation list */}
          <div className="mt-2.5 space-y-1.5 max-h-44 overflow-y-auto">
            {violations.map(v => (
              <button
                key={v.id}
                data-testid={`violation-row-${v.id}`}
                onClick={() => onSelectViolation(selectedViolationId === v.id ? null : v.id)}
                className={`w-full flex items-center justify-between px-2.5 py-2 rounded-lg border text-left transition-colors ${
                  selectedViolationId === v.id ? 'bg-navy-700 border-orange-500/40' : 'bg-navy-800 border-navy-700 hover:border-navy-500'
                }`}
              >
                <span className="text-xs text-white font-plex truncate flex-1 min-w-0">{v.name}</span>
                <span className={`flex-shrink-0 ml-2 px-1.5 py-0.5 rounded text-xs font-semibold border font-plex ${VIOLATION_STYLES[v.violation_type].badge}`}>
                  {VIOLATION_STYLES[v.violation_type].label}
                </span>
              </button>
            ))}
          </div>

          {/* Selected violation detail + notice generation */}
          {selectedViolation && (
            <div className="mt-2.5 bg-navy-800 border border-orange-500/20 rounded-xl p-3 space-y-1.5">
              {[
                { l: 'Built Area', v: `${selectedViolation.built_area_sqm} m²` },
                { l: 'Permitted Area', v: `${selectedViolation.permitted_area_sqm} m²` },
                { l: 'Detection Confidence', v: `${Math.round(selectedViolation.confidence * 100)}%` },
                { l: 'First Detected', v: selectedViolation.first_detected },
              ].map(({ l, v }) => (
                <div key={l} className="flex justify-between text-xs">
                  <span className="text-slate-400 font-plex">{l}</span>
                  <span className="text-white font-mono">{v}</span>
              ...
              {selectedViolation.violation_type !== 'compliant' && (
                <button
                  data-testid="generate-notice-btn"
                  onClick={() => onGenerateNotice(selectedViolation)}
                  className="w-full flex items-center justify-center gap-1.5 mt-2 px-3 py-2 rounded-lg bg-orange-500/15 border border-orange-500/30 text-orange-400 text-xs font-semibold font-plex hover:bg-orange-500/25 transition-colors"
                >
                  <Gavel className="w-3.5 h-3.5" /> Generate Compliance Notice
                </button>
              )}
            </div>
          )}
